...
var utilx = (function () {
    ...
    let jointStr = (status,triggerId,key) => {
        return `http://${server.ip}:${server.port}/api/`+generatorStr(`/api/trigger/control?status=${status}&triggerId=${triggerId}&key=${key}`)
    };
    var generatorStr = (params) => {
        var str = params;
        var cipher = crypto.createCipher('aes192', signature);
        var token = cipher.update(str, 'utf8', 'hex');
        token += cipher.final('hex');
